...
def _configure_styles(root: tk.Tk) -> None:
    ...
    style.map(BTN_DANGER,
        background=[("active", "#EF6A6A"), ("pressed", "#8B2A1F")],
    )

    style.configure(BTN_LINK,
        background=bg,
        foreground=accent_hi,
        bordercolor=bg,
        padding=(S_1, S_1),
...
def make_status_dot(parent: tk.Widget, level: tuple, size: int = 10) -> tk.Canvas:
    ...
    c = tk.Canvas(parent, width=size + 4, height=size + 4,
                  background=color("BG_CHROME"), highlightthickness=0)
    pad = 2
    c.create_oval(pad, pad, pad + size, pad + size,
                  fill=color(color_token), outline=color(color_token))
    return c
