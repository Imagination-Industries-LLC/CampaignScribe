...
class Tab4History(ttk.Frame):
    def __init__(self, master, app_window):
        ...
        pad = {"padx": 10, "pady": 4}
        ttk.Label(self, text="Session History",
        ...
        ttk.Button(self, text="Refresh", command=self.refresh).grid(row=1, column=2, sticky="w", **pad)

        cols = ("id", "name", "campaign", "created", "status")
        self.tree = ttk.Treeview(self, columns=cols, show="headings", height=10)
        headings = {"id": "ID", "name": "Session Name", "campaign": "Campaign",
        ...
        ttk.Entry(det, textvariable=self.name_var, width=50).grid(row=0, column=1, sticky="ew", padx=4)
        ...
        ttk.Entry(det, textvariable=self.campaign_var, width=50).grid(row=1, column=1, sticky="ew", padx=4)
        ...
        ttk.Button(det, text="Open Folder",
                   command=lambda: open_path_native(self.tr_var.get())).grid(row=4, column=2, padx=4)
        ...
        ttk.Button(reopen, text="Reopen in Transcribe",
                   command=self._reopen_transcribe).pack(side="left", padx=(0, 6))
        ttk.Button(reopen, text="Reopen in Summarize",
                   command=self._reopen_summarize).pack(side="left", padx=6)

        ttk.Button(self, text="Delete Session Record (files NOT deleted)",
                   command=self._delete).grid(row=4, column=0, columnspan=4, sticky="e", **pad)
    ...
    def refresh(self):
        ...
        for s in db.list_sessions(self.search_var.get()):
            ...
            self.tree.insert(
                "", "end", iid=str(s["id"]),
                values=(s["id"], s["display_name"], s.get("campaign_name") or "",
                        created, f"{icon} {s.get('status', '?')}"),
            )
    ...
    def _delete(self):
        ...
        for v in (self.name_var, self.campaign_var, self.status_var,
                  self.spk_var, self.tr_var, self.sum_var):
            v.set("")
        self.refresh()
